...
import data.input.KeyboardHandler;
...
public class Player extends Entity {
    ...
    public void update() {
        long now = System.currentTimeMillis();
        //boolean nextposition = GameMap.iswall();  // Read keyboard into queued direction (last pressed wins, allows buffering)
        if (keyboard.left & !map.isWall(tileCol-1,tileRow) )  { queuedDCol = -1; queuedDRow =  0; }
        if (keyboard.right & !map.isWall(tileCol+1,tileRow)) { queuedDCol =  1; queuedDRow =  0; }
        if (keyboard.up & !map.isWall(tileCol,tileRow-1))    { queuedDCol =  0; queuedDRow = -1; }
        if (keyboard.down & !map.isWall(tileCol,tileRow+1))  { queuedDCol =  0; queuedDRow =  1; }
